...
export default async function Page() {
  ...
  const chartData = rows.slice(0, 10).reverse().map(item=>({ location: item.district, probability: Math.round(item.outbreakProbability*100), confidence: Math.round(item.confidenceScore*100) }));
  return <><PageHeader eyebrow="AI prediction" title="Outbreak risk analysis" description="Use validated surveillance and environmental data to estimate district risk for the next 30 days." action={<PredictionFormDialog/>}/>
