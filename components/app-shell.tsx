...
import { Activity, Bell, BrainCircuit, CircleUserRound, ClipboardList, Database, FileText, LayoutDashboard, LogOut, Settings, ShieldCheck, Users } from "lucide-react";
...
export function AppShell({ role, name, children }: { role: keyof typeof links; name: string; children: React.ReactNode }) {
  ...
  return <TooltipProvider><SidebarProvider><Sidebar collapsible="icon">
    ...
    <SidebarFooter className="border-t border-slate-200"><SidebarMenu><SidebarMenuItem><SidebarMenuButton tooltip={name}><span className="grid size-5 place-items-center rounded bg-slate-200 text-[9px] font-semibold">{name.slice(0,2).toUpperCase()}</span><span className="truncate">{name}</span></SidebarMenuButton></SidebarMenuItem><SidebarMenuItem><SidebarMenuButton tooltip="Sign out" onClick={async()=>{await authApiRequest("/api/auth/logout");router.replace("/");router.refresh();}}><LogOut/><span>Sign out</span></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarFooter><SidebarRail/>
